...
const ORDEN_MOMENTOS = [
  'Antes desayuno', 'Después desayuno', 'Antes almuerzo', 'Después almuerzo',
  'Antes merienda', 'Antes cena', 'Después cena', 'Madrugada', 'Ayunas', 'Antes de dormir', 'Otro'
];
...
function renderPlanilla() {
  ...
  momentos.forEach(m => {
    ...
    fechas.forEach(f => {
      ...
      let contenido = '';
      const gluc = celdas.find(r => r.tipo === 'Glucemia');
      if (gluc) contenido += `<span class="celda-gluc ${claseValor(Number(gluc.valorGlucemia))}">${gluc.valorGlucemia}</span>`;
...
document.getElementById('btnGuardar').addEventListener('click', async () => {
  ...
  if (!valor && filasInsulina.length === 0) {
    mostrarToast('Cargá al menos un valor de glucemia o una insulina');
    return;
  }
