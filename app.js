...
function renderStats() {
  ...
  const sensores = items.filter(r => r.tipo === 'GlucemiaSensor').map(r => Number(r.valorGlucemia));
  ...
  if (sensores.length) {
    statSensor.style.display = '';
    document.getElementById('statSensorProm').textContent =
      Math.round(sensores.reduce((a, b) => a + b, 0) / sensores.length) + ' mg/dL';
    document.getElementById('statSensorRango').textContent =
      `Mín/Máx ${Math.min(...sensores)} / ${Math.max(...sensores)}`;
  } else {
    statSensor.style.display = 'none';
  }
}
...
function exportarPDF() {
  ...
  const glucemias = items.filter(r => r.tipo === 'Glucemia').map(r => Number(r.valorGlucemia));
  const sensores = items.filter(r => r.tipo === 'GlucemiaSensor').map(r => Number(r.valorGlucemia));
  const insulinas = items.filter(r => r.tipo === 'Insulina');
  const totalInsulina = insulinas.reduce((a, r) => a + Number(r.unidades), 0);

  const porTipo = {};
  insulinas.forEach(r => { porTipo[r.tipoInsulina] = (porTipo[r.tipoInsulina] || 0) + Number(r.unidades); });
  const desgloseInsulina = Object.entries(porTipo).map(([tipo, u]) => `${tipo}: ${u} U`).join('   ·   ');

  let y = 31;
  if (glucemias.length) {
    doc.text(`Glucemia capilar — Promedio: ${Math.round(glucemias.reduce((a, b) => a + b, 0) / glucemias.length)} mg/dL   ·   Mín/Máx: ${Math.min(...glucemias)}/${Math.max(...glucemias)} mg/dL`, 14, y);
    y += 6;
  }
  if (sensores.length) {
    doc.text(`Glucemia sensor (FreeStyle) — Promedio: ${Math.round(sensores.reduce((a, b) => a + b, 0) / sensores.length)} mg/dL   ·   Mín/Máx: ${Math.min(...sensores)}/${Math.max(...sensores)} mg/dL`, 14, y);
    y += 6;
  }
  if (insulinas.length) {
    doc.text(`Insulina total: ${totalInsulina} U   ·   ${desgloseInsulina}`, 14, y);
    y += 6;
  }
  ...
  // ---- Resumen tipo planilla (días en columna, momentos en fila) ----
  const fechasPivot = [...new Set(items.map(r => r.fecha))].sort().reverse();
  const momentosPivot = ORDEN_MOMENTOS.filter(m => items.some(r => (r.momento || 'Otro') === m));

  if (fechasPivot.length > 0 && fechasPivot.length <= 31) {
    const filasPivot = momentosPivot.map(m => {
      const fila = [m];
      fechasPivot.forEach(f => {
        const celdas = items.filter(r => r.fecha === f && (r.momento || 'Otro') === m);
        const lineas = [];
        celdas.filter(r => r.tipo === 'Glucemia').forEach(g => lineas.push(`${g.valorGlucemia} mg/dL`));
        celdas.filter(r => r.tipo === 'GlucemiaSensor').forEach(g => lineas.push(`${g.valorGlucemia} mg/dL (sensor)`));
        celdas.filter(r => r.tipo === 'Insulina').forEach(i => lineas.push(`${i.tipoInsulina} ${i.unidades}U`));
        fila.push(lineas.join('\n'));
      });
      return fila;
    });
    ...
    doc.setFontSize(14);
    doc.setTextColor(0);
    doc.text('Resumen por día', 14, 18);
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(tituloRango(), 14, 25);
    ...
    doc.autoTable({
      startY: 31,
      head: [['Momento', ...fechasPivot.map(f => `${f.slice(8, 10)}/${f.slice(5, 7)}`)]],
      body: filasPivot,
      styles: { fontSize: 7.5, cellPadding: 2, valign: 'middle' },
      headStyles: { fillColor: [21, 94, 99] },
      columnStyles: { 0: { fontStyle: 'bold', cellWidth: 26 } }
    });
  } else if (fechasPivot.length > 31) {
    doc.addPage();
    doc.setFontSize(11);
    doc.setTextColor(100);
    doc.text('El resumen por día no se incluye en períodos de más de 31 días.', 14, 18);
    doc.text('Elegí "Este mes" o un rango más acotado para verlo.', 14, 25);
  }

  // ---- Comidas registradas ----
  const comidas = items.filter(r => r.tipo === 'Comida');
  if (comidas.length > 0) {
    doc.addPage();
    doc.setFontSize(14);
    doc.setTextColor(0);
    doc.text('Comidas registradas', 14, 18);
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(tituloRango(), 14, 25);
    ...
    doc.autoTable({
      startY: 31,
      head: [['Fecha', 'Hora', 'Momento', 'Comida']],
      body: comidas.map(c => [c.fecha, c.hora, c.momento || '', c.observaciones || '']),
      styles: { fontSize: 9 },
      headStyles: { fillColor: [61, 131, 97] }
    });
  }
  ...
  if (chart) {
    ...
    doc.setFontSize(14);
    doc.setTextColor(0);
    doc.text('Evolución de glucemia', 14, 18);
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(tituloRango(), 14, 25);
    try {
      const img = chart.toBase64Image('image/png', 1.0);
      doc.addImage(img, 'PNG', 14, 32, 180, 95);
    } catch (e) { /* si falla la captura del gráfico, se omite sin romper el PDF */ }
  }
